function [RecordedData,index] = grabData(ljudObj,ljhandle,DataStruct)
try
    ...
    [ljerror, test] = ljudObj.eGet(ljhandle, LabJack.LabJackUD.IO.START_STREAM, 0, 0, 0);
    ...
    while toc<=0.5
        ...
        for  j= 1:PacketLength
            %             NewData=DataStruct
            disp(DataStruct(1:10))
            RecordedData(index+(j-1))=DataStruct(j+1);
            %             ecordedData(index+(j-1)/NumberOfChannels,k)=data(j+k-1);
        end
